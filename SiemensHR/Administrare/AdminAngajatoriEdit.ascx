<%@ Control Language="c#" AutoEventWireup="false" Codebehind="AdminAngajatoriEdit.ascx.cs" Inherits="SiemensHR.Administrare.AdminAngajatoriEdit" TargetSchema="http://schemas.microsoft.com/intellisense/ie5"%>
...
function CheckDeleteAngajator( text )
{
	for(var i=0; i<arAngajatorID.length; i++)
	{	
		if (<%Response.Write(this.idAngajator);%>== Number(arAngajatorID[i]))
		{
			if (Number(arNoAngajati[i])!=0)
			{
				alert("Angajatorul nu poate fi sters!\nExista angajati asociati acestuia!");
				return false;
			}
		}
	}
...
function FillDrpDomDeActPrincipal()
{
	...
	for(i=0;i<domeniiAngajatorList.options.length;i++)
	{
		...
		//sau
		//domeniuPrincipalList.add(o , i);
		//sau
		//var theOption=document.createElement("OPTION");
		//theText=document.createTextNode(selText);
        //theOption.appendChild(theText);
        //theOption.setAttribute("value",selValue);
        //domeniuPrincipalList.appendChild(theOption);
	}
...
function DeleteDomDeActSelectat()
{
	...
	if(domeniiAngajatorList.selectedIndex>=0)
	{
		...
		domeniiList.add( opt);
		//domeniiList.add( opt, domeniiList.options.length-1 );
		//sau
		//domeniiList.options[domeniiList.options.length] = opt;
...
function DeleteAllDomDeAct()
{
	...
	if(domeniiAngajatorList.options.length!=0)
	{
		...
		for(i=0;i<domeniiAngajatorList.options.length;i++)
		{
			...
			var opt = new Option(selText, selValue);
			domeniiList.add(opt);
			//domeniiList.add(opt,i);
			//sau
			//domeniiList.options[i] = opt;
		}
...
//Functie apelata atunci cand se schimba tipul de completare al carnetului de munca
function lstTipCompletareChange( tipCompletare)
{
	document.getElementById( txtTipCompletareHiddenClient).value = tipCompletare.value;
}
...
{
	var judeteCombo = document.getElementById( lstDJudetSectorClient );
...
function FillLstDJudet( index )
{
	...
	var judCombo = document.getElementById( lstDJudetSectorClient );
...
function AddPunctLucru()
{
	...
	var txtCUIClient = document.getElementById( txtCUI );
	var txtNrCrtSediuSecundarClient = document.getElementById( txtNrCrtSediuSecundar );
	...
	else
	{
		...
		else
		{
			...
			var opt = new Option(txtNumePunctLucruClient.value + ',' + drpCaseAsig.options[drpCaseAsig.selectedIndex].text + ',' + txtCUIClient.value + ',' + txtNrCrtSediuSecundarClient.value);
			...
			txtCUIClient.value = '';
			txtNrCrtSediuSecundarClient.value = '';
			lstPunctLucruClient.selectedIndex = -1;
		}
	}
}

//Oprescu Claudia
//sterge un punct de lucru
//Modificat: Lungu Andreea 26.05.2008 - a fost adaugata o casa de asigurari implicita pentru un punct de lucru
function DeletePunctLucru()
{
	...
	else
	{
		...
		PunctLucru = sirPunctLucru[lstPunctLucruClient.selectedIndex].split(",");
		if (PunctLucru[6] == "nu")
		{
			alert("Punctul de lucru " + PunctLucru[2] + " nu poate fi sters pentru ca exista date corespunzatoare lui!");
		}
		else
		{
...
//Oprescu Claudia
//sterge toate Punctele de lucru
...
function DeleteAllPunctLucru()
{
	...
	{
		lstPunctLucruClient = document.getElementById( lstPunctLucru );
		sirPunctLucru = document.getElementById( txtHiddenPunctLucru ).value.split(";");
		for (i=0; i<sirPunctLucru.length; i++)
		{
			PunctLucru = sirPunctLucru[i].split(",");
			if (PunctLucru[6] == "nu")
			{
				if (PunctLucruNesterse == "")
					PunctLucruNesterse = PunctLucru[2];
				else
					PunctLucruNesterse += "," +PunctLucru[2]; 
			}
			else
			{
				j=0;
				while (j<lstPunctLucruClient.length)
				{
					if (PunctLucru[2] + "," + PunctLucru[3] + "," + PunctLucru[4] + "," + PunctLucru[5] == lstPunctLucruClient.options[j].text)
						lstPunctLucruClient.remove(j);
					j++;
				}	
			}
		}
		...
		if (PunctLucruNesterse != "")
			alert("Nu au putut fi sterse urmatoarele puncte de lucru: " + PunctLucruNesterse + " pentru ca exista inregistrari atasate lor!");
		txtNumePunctLucruClient.value = '';
		ChangePunctLucruListSterge();
	}		
}
...
function ChangePunctLucruListSterge()
{
	...
	if(lstPunctLucruClient.length!=0)
	{
		...
		{	
			...
			{
				alert("-" + lstPunctLucruClient.options[j].text + "-" + PunctLucru[2] + "," + PunctLucru[3] + "," + PunctLucru[4] + "," + PunctLucru[5] + "-");
				if (lstPunctLucruClient.options[j].text == PunctLucru[2] + "," + PunctLucru[3] + "," + PunctLucru[4] + "," + PunctLucru[5])
				{
					alert('sunt egale');
				
					//textPunctLucruList+=lstPunctLucruClient.options[j].value + "," + lstPunctLucruClient.options[j].text+"," + PunctLucru[2] + ";";
					textPunctLucruList+=PunctLucru[0]+ "," + PunctLucru[1]+ "," + lstPunctLucruClient.options[j].text+"," + PunctLucru[6] + ";";
				}
			}
			if (lstPunctLucruClient.options[lstPunctLucruClient.length-1].text == PunctLucru[2] + "," + PunctLucru[3] + "," + PunctLucru[4] + "," + PunctLucru[5])
			{
				...
				textPunctLucruList+=PunctLucru[0]+ "," + PunctLucru[1]+ "," + lstPunctLucruClient.options[lstPunctLucruClient.length-1].text+"," + PunctLucru[6];
			}
		}
...
function ChangePunctLucruListAdauga()
{
	...
	if(lstPunctLucruClient.length!=0)
	{
		if (document.getElementById(txtHiddenPunctLucru).value == "")
			document.getElementById(txtHiddenPunctLucru).innerText = "," + drpCaseAsig.options[drpCaseAsig.selectedIndex].value + "," + lstPunctLucruClient.options[lstPunctLucruClient.length-1].text +",da";	
		else
			document.getElementById(txtHiddenPunctLucru).innerText = document.getElementById(txtHiddenPunctLucru).value + ";" + "," + drpCaseAsig.options[drpCaseAsig.selectedIndex].value + ","  + lstPunctLucruClient.options[lstPunctLucruClient.length-1].text +",da";	
	}
}

// Lungu Andreea - 16.02.2011
// In cazul in care este selectat un punct de lucru sunt completate controalele.
function FillPunctLucruControls()
{
	var drpCaseAsigClient = document.getElementById( drpCaseDeAsigClientID );
	var lstPunctLucruClient = document.getElementById( lstPunctLucru );
	var txtNumePunctLucruClient = document.getElementById( txtNumePunctLucru );
	var txtCUIClient = document.getElementById( txtCUI );
	var txtNrCrtSediuSecundarClient = document.getElementById( txtNrCrtSediuSecundar );

	var sirDatePuncteDeLucru = new Array();
	var sirDatePunctDeLucru = new Array();
	var index = lstPunctLucruClient.selectedIndex;
	
	//18,1,Brasov,Casa de Asigurari Brasov - CAS,,,nu
	sirDatePuncteDeLucru = document.getElementById( txtHiddenPunctLucru ).value.split(";")
	sirDatePunctDeLucru = sirDatePuncteDeLucru[index].split(",");
	
	for(index = 0; index < drpCaseAsigClient.length; index++)
    {
		if (drpCaseAsigClient[index].value == sirDatePunctDeLucru[1])
		{
               drpCaseAsigClient.selectedIndex = index;
               index = drpCaseAsigClient.length;
        }
    }    

	txtNumePunctLucruClient.value = sirDatePunctDeLucru[2];
	txtCUIClient.value = sirDatePunctDeLucru[4];
	txtNrCrtSediuSecundarClient.value = sirDatePunctDeLucru[5];
}

// Lungu Andreea - 16.02.2011
// Se modifica datele despre punctul de lucru selectat.
function UpdatePunctLucru()
{
	var lstPunctLucruClient = document.getElementById( lstPunctLucru );
	var drpCaseAsigClient = document.getElementById( drpCaseDeAsigClientID );
	var txtNumePunctLucruClient = document.getElementById( txtNumePunctLucru );
	...
	if (lstPunctLucruClient.selectedIndex >= 0)
	{
		lstPunctLucruClient[lstPunctLucruClient.selectedIndex].text = txtNumePunctLucruClient.value + ',' + drpCaseAsigClient.options[drpCaseAsigClient.selectedIndex].text + ',' + txtCUIClient.value + ',' + txtNrCrtSediuSecundarClient.value;
		
		ChangePunctLucruListUpdate();
	...
	{
		alert("Trebuie sa selectati un punct de lucru!");
	}
}

//se actualizeaza campul care retine lista cu puncte de lucru
//Lungu Andreea 16.02.2011 - s-au facut modificari deoarece pentru un anumit punct de lucru s-a adaugat modificare
function ChangePunctLucruListUpdate()
{
	var lstPunctLucruClient = document.getElementById( lstPunctLucru );
	var drpCaseAsigClient = document.getElementById( drpCaseDeAsigClientID );
	
	var idPunctLucruModificat = lstPunctLucruClient[lstPunctLucruClient.selectedIndex].value;
	var idCasaAsigModificat = drpCaseAsigClient[drpCaseAsigClient.selectedIndex].value;
	//alert('id de modif:' + idPunctLucruModificat);
	//alert('id casa de asig de modif:' + idCasaAsigModificat);
	...
	if(lstPunctLucruClient.length!=0)
	{
		...
		{	
			...
			for(j=0;j<lstPunctLucruClient.length-1;j++)
			{
				if (PunctLucru[0] == idPunctLucruModificat)
				{
						textPunctLucruList+=PunctLucru[0]+ "," + idCasaAsigModificat + "," + lstPunctLucruClient.options[j].text+"," + PunctLucru[6] + ";";
				}
				else
				{
					textPunctLucruList+=PunctLucru[0]+ "," + PunctLucru[1]+ "," + lstPunctLucruClient.options[j].text+"," + PunctLucru[6] + ";";
			...
			if (PunctLucru[0] == idPunctLucruModificat)
			{
				textPunctLucruList+=PunctLucru[0]+ "," + idCasaAsigModificat+ "," + lstPunctLucruClient.options[lstPunctLucruClient.length-1].text+"," + PunctLucru[6];
			}
			else
			{
				textPunctLucruList+=PunctLucru[0]+ "," + PunctLucru[1]+ "," + lstPunctLucruClient.options[lstPunctLucruClient.length-1].text+"," + PunctLucru[6];
...
<form id="Form1" name="Form1" method="post" runat="server">
		...
		<tr>
			<td align="center">
				<table cellSpacing="1" cellPadding="0" border="0">
					...
					<tr>
						<td class="BigBlueBold" align="center" colSpan="3" height="50">Editeaza angajator</td>
					</tr>
					<tr>
						<td align="center" colSpan="3"><asp:table id="tableTabs" runat="server"></asp:table></td>
					<tr>
						<td id="tableDateAngajator" vAlign="top" align="center">
							<table class="tabBackground" cellSpacing="1" cellPadding="0" width="735" border="0">
								...
								<TR>
									...
									<TD><asp:textbox id="txtNrInregItm" runat="server" CssClass="NormalEditBoxuri" MaxLength="25" Width="200px"></asp:textbox><SPAN class="CommentRedBold">*</SPAN>
										<asp:requiredfieldvalidator id="requiredNrInregItm" runat="server" CssClass="AlertRedBold" ErrorMessage="Completati numarul de inregistrare ITM!"
											ControlToValidate="txtNrInregItm"><</asp:requiredfieldvalidator><asp:regularexpressionvalidator id="regularNrInregItm" runat="server" CssClass="AlertRedBold" ErrorMessage="Numarul de inregistrare ITM nu poate sa contina caractere invalide: <>()';&quot;\ "
											ControlToValidate="txtNrInregItm" ValidationExpression="^[^>|(|)|<|'|&quot;|;|\\]*$"><</asp:regularexpressionvalidator></TD>
								</TR>
								<tr>
									<td class="GreenSeparator" colSpan="2"><IMG height="1" src="images/1x1.gif"></td>
								</tr>
								...
								<TR>
									...
									<TD><asp:textbox id="txtZiLichidareSalar" runat="server" CssClass="NormalEditBoxuri" MaxLength="2"
											...
											ControlToValidate="txtZiLichidareSalar" MaximumValue="31" MinimumValue="1" Type="Integer"><</asp:rangevalidator></TD>
								</TR>
								<tr>
									<td class="GreenSeparator" colSpan="2"><IMG height="1" src="images/1x1.gif"></td>
								</tr>
								<TR>
									...
									<TD><asp:dropdownlist id="lstTipCompletare" runat="server" CssClass="SelectStyle" Width="400px" onChange="lstTipCompletareChange(this);"></asp:dropdownlist></TD>
								</TR>
							</table>
						</td>
							...
							<table class="tabBackground" cellSpacing="1" cellPadding="0" width="735" border="0">
								...
								<tr>
									...
									<td><asp:textbox id="txtNumarContVechi" runat="server" CssClass="NormalEditBoxuri" MaxLength="16"
											Width="296px"></asp:textbox><SPAN class="CommentRedBold"></SPAN></td>
								</tr>
								<tr>
									<td class="GreenSeparator" colSpan="2"><IMG height="1" src="images/1x1.gif"></td>
								</tr>
								...
								<tr>
									<td align="center" colSpan="2"><asp:listbox id="lstConturiBancare" runat="server" Width="645px" OnChange="javascript:FillContBancarControls();"
											Height="154px"></asp:listbox><asp:requiredfieldvalidator id="requiredConturi" runat="server" CssClass="AlertRedBold" ErrorMessage="Nu ati completat nici un cont pentru angajator!"
								...
								<tr>
									<td align="center" colSpan="2"><INPUT class="ButtonStyle" id="btnDeleteCont" onclick="javascript:DeleteContBancarConfirmare()"
											type="button" value="Sterge cont">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										<INPUT class="ButtonStyle" id="btnUpdateCont" onclick="javascript:UpdateContBancar()" type="button"
											value="Modifica date cont">
									</td>
									<td></td>
								</tr>
							</table>
							<INPUT id="txtListaConturiHidden" style="DISPLAY: none" name="txtListaConturiHidden" runat="server">
						</td>
						<td id="tableAdresa" vAlign="top" align="center">
							<table class="tabBackground" cellSpacing="1" cellPadding="0" width="735" border="0">
								...
								<TR>
									...
									<TD><asp:dropdownlist id="lstDJudetSector" runat="server" CssClass="SelectStyle" Width="150px" onChange="lstDJudetSectorOnChange(this);"></asp:dropdownlist><asp:requiredfieldvalidator id="requiredDJudet" runat="server" CssClass="AlertRedBold" ErrorMessage="Completati judetul!"
											ControlToValidate="lstDJudetSector" Height="10px"><</asp:requiredfieldvalidator></TD>
								</TR>
								<tr>
									<td class="GreenSeparator" colSpan="2"><IMG height="1" src="images/1x1.gif"></td>
								</tr>
								<TR>
									...
									<TD><asp:textbox id="txtDSector" runat="server" CssClass="NormalEditBoxuri" MaxLength="5" Width="130px"></asp:textbox><SPAN class="CommentRedBold"></SPAN><asp:regularexpressionvalidator id="regularSector" runat="server" CssClass="AlertRedBold" ErrorMessage="Sectorul nu poate sa contina caractere invalide: <>()';&quot;\ "
						...
						<td id="tableDomeniiDeActivitate" vAlign="top" align="center">
							<table class="tabBackground" cellSpacing="1" cellPadding="0" width="735" border="0">
								...
								<tr>
									<td style="WIDTH: 286px" vAlign="middle" align="center"><asp:listbox id="lstBoxDomenii" runat="server" CssClass="SelectStyle" Width="348px" Height="150px"
									...
									<td vAlign="middle" align="center"><asp:listbox id="lstBoxDomeniiAngajator" runat="server" CssClass="SelectStyle" Width="350px"
								...
								<TR>
									<TD class="NormalGreenBold" style="WIDTH: 286px" vAlign="middle" align="center"><INPUT id="txtHiddenDomDeActListID" name="txtHiddenDomDeActListID" runat="server">
									</TD>
									<TD align="center"></TD>
								...
								<TR>
									<TD class="NormalGreenBold" style="WIDTH: 286px" vAlign="middle" align="center"><INPUT id="txtHiddenDomDeActPpID" name="txtHiddenDomDeActPpID" runat="server">
									</TD>
									<TD align="center"></TD>
									<TD class="NormalGreenBold" vAlign="middle" align="center"><SELECT id="drpDomDeActPrincipal" style="WIDTH: 345px; HEIGHT: 20px" onchange="ChangeDomDeActPp(this.options[this.selectedIndex].value)"
						...
						<td id="tablePunctLucru" vAlign="top" align="center">
							<TABLE class="tabBackground" cellSpacing="1" cellPadding="0" width="735" border="0">
								<TBODY>
									<TR>
										<TD class="NormalGreenBold" style="WIDTH: 283px; HEIGHT: 18px" vAlign="middle" align="center">Punct 
											de lucru</TD>
										<TD class="NormalGreenBold" style="WIDTH: 71px; HEIGHT: 18px" align="center"></TD>
										<TD class="NormalGreenBold" style="HEIGHT: 18px" vAlign="middle" align="center">
											<P>Puncte de lucru&nbsp;pentru angajator</P>
										</TD>
									</TR>
									<TR>
										<TD class="NormalGreenBold" style="WIDTH: 283px; HEIGHT: 18px" vAlign="middle" align="center">(Denumire, 
											Casa de asigurari)</TD>
										<TD class="NormalGreenBold" style="WIDTH: 71px; HEIGHT: 18px" align="center"></TD>
										<TD class="NormalGreenBold" style="HEIGHT: 18px" vAlign="middle" align="center">(Denumire, 
											Casa de asigurari)</TD>
									</TR>
									<TR>
										<TD class="GreenSeparator" colSpan="3"><IMG height="1" src="images/1x1.gif"></TD>
									...
									<TR>
										<TD style="WIDTH: 283px; HEIGHT: 167px" vAlign="top" align="center">&nbsp;
											<TABLE id="Table1" style="WIDTH: 279px; HEIGHT: 77px" cellSpacing="0" cellPadding="2" width="279"
												border="0">
												<TR>
													<TD class="NormalGreenBold">Punct de lucru:</TD>
												</TR>
												<TR>
													<TD><asp:textbox id="txtNumePunctLucru" runat="server" Width="278px"></asp:textbox></TD>
												</TR>
												<TR>
													<TD class="NormalGreenBold">CUI:</TD>
												</TR>
												<TR>
													<TD>
														<asp:textbox style="Z-INDEX: 0" id="txtCUI" runat="server" Width="278px"></asp:textbox></TD>
												</TR>
												<TR>
													<TD class="NormalGreenBold">Nr. crt. sediu secundar</TD>
												</TR>
												<TR>
													<TD class="NormalGreenBold">
														<asp:textbox style="Z-INDEX: 0" id="txtNrCrtSediuSecundar" runat="server" Width="278px"></asp:textbox></TD>
												</TR>
												<TR>
													<TD class="NormalGreenBold">Selectati casa de asigurari implicita:</TD>
												</TR>
												<TR>
													<TD><asp:dropdownlist id="drpCaseDeAsigurari" runat="server" Width="278px"></asp:dropdownlist></TD>
												</TR>
											</TABLE>
										</TD>
										<TD style="WIDTH: 71px; HEIGHT: 167px" align="center">
											<TABLE>
												<TBODY>
													<TR>
											...
											<TABLE>
												<TBODY>
													<TR>
														<TD align="center"><INPUT style="Z-INDEX: 0" id="btnModifyOnePunctLucru" class="ButtonStyle" onclick="javascript:UpdatePunctLucru()"
																value="Modifica" type="button" name="btnModifyOnePunctLucru">
														</TD>
													</TR>
												</TBODY>
											</TABLE>
										...
										<TD style="HEIGHT: 151px" vAlign="middle" align="center">
											<table style="HEIGHT: 200px" width="100%" align="center">
												<TBODY>
													<tr>
														<td style="HEIGHT: 106px" vAlign="middle" align="center" colSpan="3"><asp:listbox id="lstPunctLucru" runat="server" CssClass="SelectStyle" Width="95%" Rows="10" OnChange="javascript:FillPunctLucruControls();"></asp:listbox><asp:requiredfieldvalidator id="requiredPuncteLucru" runat="server" CssClass="AlertRedBold" ErrorMessage="Nu ati completat nici un punct de lucru!"
									...
									<tr>
										<td align="center" colSpan="3"><INPUT id="txtHiddenPunctLucru" name="txtHiddenPunctLucru" runat="server"></td>
									</tr>
								</TBODY>
							</TABLE>
						</td>
						<td id="tableReprezLegal" vAlign="top" align="center"><TABLE class="tabBackground" cellSpacing="1" cellPadding="0" width="735" border="0">
								<TBODY>
									...
									<tr>
										<td align="center" colSpan="3" style="DISPLAY: none"><INPUT id="txtHiddenNrFunctii" style="WIDTH: 157px; HEIGHT: 22px" name="txtHiddenNrFunctii"
												runat="server"><INPUT id="txtHiddenReprez" style="WIDTH: 207px; HEIGHT: 22px" size="29" name="txtHiddenReprez"
												runat="server"></td>
									</tr>
								</TBODY>
							</TABLE>
					...
					<TR>
						<td colSpan="3">
							<table width="100%">
								<tr>
									<td class="CommentRedBold" align="left">* - Campuri obligatorii
									</td>
					...
					<tr>
						<td align="center" colSpan="2" height="50"><asp:button id="btnSave" onmouseover="MouseOverButton(this)" onmouseout="MouseOutButton(this)"
							...
							<asp:button id="btnSterge" onmouseover="MouseOverButton(this)" onmouseout="MouseOutButton(this)"
								runat="server" CssClass="ButtonStyle" Text="  Sterge   " Visible="False" CausesValidation="False"></asp:button>&nbsp;
	...
	<input 
type=hidden value="<%Response.Write(this.idAngajator);%>" name=txtAngajatorID> <input id="txtTaraHidden" type="hidden" name="txtTaraHidden" runat="server">
	<input id="txtJudetSectorHidden" type="hidden" name="txtJudetSectorHidden" runat="server">
	<input id="txtTipCompletareHidden" type="hidden" name="txtTipCompletareHidden" runat="server">
</form>
<script>
